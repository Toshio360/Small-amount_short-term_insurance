...
・EurekaServerにAPIインスタンスを登録します。</br>
・[ガイドはこちら](docs/guide.md)
...
2. realmとclientの設定をしていない場合は設定をする、my-realmにユーザーを登録する<br>
    realm名`my-realm` Username`my-realm-user`<br>
    Client ID`my-client`<br>
    Clients > Client details > Settings の Valid redirect URIs に`http://localhost:8088/login/oauth2/code/keycloak`を入力する<br>
